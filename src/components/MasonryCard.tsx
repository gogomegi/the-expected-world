...
export function MasonryCard({
  ...
  return (
    <Link
      ...
      className={`masonry-item block bg-surface border border-divider rounded-md overflow-hidden cursor-pointer transition-all duration-[400ms] ease-[cubic-bezier(0.4,0,0.2,1)] hover:border-brass-muted hover:shadow-[0_8px_40px_rgba(0,0,0,0.4)] hover:-translate-y-1 ${featured ? "bg-gradient-to-br from-surface to-[rgba(201,168,76,0.05)] border-brass-muted" : ""}`}
    >
      {imageUrl && (
        <img
          src={imageUrl}
      ...
      <div className={featured ? "p-7" : "p-5"}>
        ...
        <div
          className={`font-quote leading-[1.5] mb-3.5 line-clamp-4 ${featured ? "text-[22px] line-clamp-6" : "text-[17px]"}`}
        >
